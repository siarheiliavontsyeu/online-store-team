import { ProductI, ViewOptions } from '../../../../../constants/types';

const getRatingStars = (rating: number) =>
  rating < 4.5 ? '<i class="fas fa-star-half-alt"></i>' : '<i class="fas fa-star"></i>';

export const renderProductCard = (card: ProductI, viewBy: ViewOptions) => {
  ...
  const bigCardTemplate = ` <ul class="list-group list-group-flush">
  <li class="list-group-item">
    <p class="text-muted mb-0">Available: <span class="fw-bold">${card.stock}</span></p>
  </li>
</ul>`;
  const bigCardTemplateSecond = `
  ...
  ${viewBy === 'Big-cards' ? bigCardTemplate : ''}
  <div class="card-body">
    ${viewBy === 'Big-cards' ? bigCardTemplateSecond : ''}
